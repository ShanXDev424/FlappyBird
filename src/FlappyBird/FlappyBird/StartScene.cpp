...
StartScene::StartScene()
{
	...
	this->addChild(bird);
	//==================新增====================
	auto exit = gcnew Sprite(ResLoader::getImage(L"exit"));
	//创建结束按钮正常状态和按下状态时的精灵显示
	auto exit2 = gcnew  Sprite(ResLoader::getImage(L"exit"));
	exit2->setPosY(5);//按钮被选中时向下移动一点点
    auto button = gcnew Button (exit,exit2);
	//创建结束按钮
	button->setClickFunc([]
		{
			Game::quit();//按下结束按钮，退出游戏
		});
	button->setPosX(Window::getWidth() / 2);
	button->setPosY(Window::getHeight() - exit ->getHeight()+10 );
	...
	shareBtn->setClickFunc([]
		{
			ResLoader::playMusic(MusicType::Click);
			ShellExecute(NULL, L"open", L"https://baidu.com", NULL, NULL, SW_SHOWNORMAL);
		});
